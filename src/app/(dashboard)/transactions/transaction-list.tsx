...
export function TransactionList({ initialData }: { initialData: any[] }) {
  ...
  const toggleSelectAll = () => {
    const unassignedItems = initialData.filter(item => !item.reimbursement_id)
    if (selectedIds.length === unassignedItems.length) {
      setSelectedIds([])
    } else {
      setSelectedIds(unassignedItems.map(item => item.id))
    }
  }
  ...
  const unassignedItems = initialData.filter(item => !item.reimbursement_id)
  const allSelected = unassignedItems.length > 0 && selectedIds.length === unassignedItems.length
    ...
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden relative min-h-[400px]">
      ...
      {initialData.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h3 className="text-lg font-medium text-slate-900 mb-1">Belum ada nota</h3>
          <p className="text-slate-500 mb-6">Anda belum pernah menginput nota apapun.</p>
